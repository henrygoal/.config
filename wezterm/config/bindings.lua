...
local keys = {
    ...
    -- tabs --
    -- tabs: spawn+close
    { key = 't', mods = mod.SUPER,     action = act.SpawnTab({ DomainName = 'local' }) },
    ...
    { key = ']', mods = mod.SUPER_REV, action = act.MoveTabRelative(1) },
    { key = 'l', mods = mod.SUPER_REV, action = act.ShowTabNavigator },
    ...
    { key = 'k',    mods = 'CTRL|SHIFT', action = act.ActivatePaneDirection('Right') },
    {
        key = 'i',
        mods = mod.SUPER_REV,
...
local key_tables = {
    ...
    resize_pane = {
        ...
        { key = 'k',      action = act.AdjustPaneSize({ 'Right', 1 }) },
        { key = 'Escape', action = 'PopKeyTable' },
        { key = 'q',      action = 'PopKeyTable' },
    },
}
